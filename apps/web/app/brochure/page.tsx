  ...
  return (
    <Section eyebrow="Brochure" title="InsuCARE Company Brochure" intro="Use browser print to save this production-ready brochure as PDF.">
      <div className="rounded-[2rem] border border-oxblood/10 bg-white p-8 shadow-sm print:shadow-none">
        ...
        <p className="mt-3 text-ink/70">{serviceCategories.join(", ")}</p>
        <h3 className="mt-8 font-display text-3xl font-bold">Claims Assistance</h3>
        <p className="mt-3 text-ink/70">Claim notification, documentation, insurer coordination, survey assistance, settlement follow-up and closure.</p>
        <h3 className="mt-8 font-display text-3xl font-bold">Insurance Partners</h3>
        <p className="mt-3 text-ink/70">{partners.map((partner) => partner.name).join(", ")}</p>
        <h3 className="mt-8 font-display text-3xl font-bold">Contact</h3>
        <p className="mt-3 text-ink/70">{company.phone} · {company.email} · {company.addressLines.join(", ")}</p>
      </div>
    </Section>
  );
}
