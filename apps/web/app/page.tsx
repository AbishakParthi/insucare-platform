    ...
    <>
              ...
              <div className="rounded-[2rem] bg-white p-6 text-ink">
                ...
                <div className="mt-8 grid gap-4">
                  {["Coverage gap review", "Multi-insurer comparison", "Claims desk coordination"].map((item, index) => (
                    <div key={item} className="rounded-2xl border border-oxblood/10 bg-porcelain p-5">
      ...
      <Section title="Trusted Numbers" intro="Built for individuals and businesses who want transparent advice, responsive service and confident claims support.">
        <div className="grid gap-5 md:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-[2rem] border border-oxblood/10 bg-white p-7 shadow-sm">
      ...
      <Section eyebrow="Why InsuCARE" title="Advice before policy. Support after purchase.">
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {whyChooseUs.map((item) => (
            <div key={item} className="group rounded-[2rem] border border-oxblood/10 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-premium">
      ...
      <Section eyebrow="Featured Services" title="Insurance programs for business and life." className="bg-porcelain">
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {featured.map((service) => (
            <Link key={service.slug} href={`/services/${service.slug}`} className="rounded-[2rem] bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-premium">
      ...
      <Section eyebrow="Claims Assistance" title="A guided claims workflow from notification to closure.">
        <div className="grid gap-4 md:grid-cols-3">
          {claimSteps.map((step, index) => (
            <div key={step} className="rounded-[1.6rem] border border-oxblood/10 bg-white p-6">
      ...
      <Section eyebrow="Client Voices" title="Trusted by business owners, HR leaders and families." className="bg-ink text-white">
      ...
      <Section eyebrow="Insurance Partners" title="Multi-insurer access for better comparison.">
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
          {partners.slice(0, 12).map((partner) => (
            <div key={partner.name} className="grid min-h-24 place-items-center rounded-2xl border border-oxblood/10 bg-white p-4 text-center text-sm font-extrabold text-ink/75">
              {partner.name}
            </div>
          ))}
        </div>
      ...
      <Section eyebrow="Latest Blogs" title="Insurance insights for better decisions." className="bg-porcelain">
        <div className="grid gap-5 md:grid-cols-3">
          {blogs.map((post) => (
            <Link key={post.slug} href={`/blogs/${post.slug}`} className="rounded-[2rem] bg-white p-7 shadow-sm">
              ...
              <p className="mt-3 text-sm leading-7 text-ink/65">{post.excerpt}</p>
            </Link>
          ))}
        </div>
      </Section>
